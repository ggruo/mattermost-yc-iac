...
Если `.venv/bin/activate` отсутствует (например, редактор не выполнил
`postCreateCommand`), запустите инициализацию вручную из корня репозитория
...
Скрипт можно запускать повторно; при ошибке установки пакетов сначала устраните
её и повторите команду. Само подключение volume `.venv` не создаёт Python-окружение.
